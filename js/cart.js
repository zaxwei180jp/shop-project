...
async function init() {
  ...
  const rows = ids.map(cartKey => {
    ...
    // 默认使用台幣價格或特價
    let unitPrice = p.tprice || p.price;
    ...
    return `
      <div style="display:flex;gap:12px;padding:16px;border-bottom:1px solid var(--gray-200);background:var(--white)">
        ...
        <div style="flex:1;min-width:0">
          ...
          <div style="font-size:16px;font-weight:700;margin-bottom:8px">${formatPrice(unitPrice)}${appliedCoupon && p.sprice ? `<span style="font-size:12px;color:var(--gray-400);text-decoration:line-through;margin-left:6px">${formatPrice(p.sprice)}</span>` : ""}</div>
