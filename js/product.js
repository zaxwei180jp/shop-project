...
(async () => {
  ...
  wrap.innerHTML = `
    ...
    <div id="pdBreadcrumb" style="padding:12px 16px;font-size:13px;color:var(--gray-400);display:flex;gap:6px;align-items:center">
      ...
      ${p.mainCategory ? `<span>›</span><a href="index.html?main=${encodeURIComponent(p.mainCategory)}" style="color:var(--gray-400);text-decoration:none">${p.mainCategory}</a>` : ""}
    ...
    <!-- 上一件 / 下一件 -->
    <div id="pdPrevNext" style="display:grid;grid-template-columns:1fr 1fr;gap:1px;background:var(--gray-200);margin-top:8px">
